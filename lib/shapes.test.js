...
test("Logo Shape Circle", () =>{
    ...
    expect(generate_svg(data)).toEqual(`
    <svg width="300" height="200">
        <circle cx="150" cy="100" r="100" fill="${data.color.toLowerCase()}"></circle>
...
test("Logo Shape Triangle", () =>{
    ...
    expect(generate_svg(data)).toEqual(`
    <svg width="300" height="200">
        <polygon points="150,0 300,200 0,200" fill="${data.color.toLowerCase()}"></polygon>
        <text x="150" y="150" alignment-baseline="middle" fill="${data.font.toLowerCase()}" text-anchor="middle" font-size="20">${data.logo}</text>
    </svg>`
    )
});
